...
data_root = os.path.abspath('..')#获得当前工作目录
...
def get_global(X, strategy):
    min_value = X.min().min()
    X = X.fillna(min_value)
    return X

def get_pair(X, sample_name, fill_strategy=None, multiple_thread=False):
    sub_sample_list = {}

    all_sample_num = X.shape[0]
    # all_sample_num = 5
    ...
    if multiple_thread:
        ...
        for sample_id in range(all_sample_num-1):
            for other_sample_id in range(sample_id+1, all_sample_num):
                _sample = X.iloc[[sample_id, other_sample_id], :]
                sub_sample_ids = (sample_name[sample_id], sample_name[other_sample_id])
                res = pool.apply_async(get_fill_data, args=(_sample, sub_sample_ids, fill_strategy,))
                sub_sample_list[sub_sample_ids] = res
        ...
        for sample_id in range(all_sample_num-1):
            for other_sample_id in range(sample_id+1, all_sample_num):
                _sample = X.iloc[[sample_id, other_sample_id], :]
                sub_sample_ids = (sample_name[sample_id], sample_name[other_sample_id])
                res = get_fill_data(_sample, sub_sample_ids, fill_strategy)
                sub_sample_list[sub_sample_ids] = res
    for k, v in sub_sample_list.items():
        sub_sample_list[k] = v.get()
    print('data pair procession done time:%s s' % (time.time() - start_time))
    return sub_sample_list

def get_fill_data(sub_sample, sub_sample_ids, fill_strategy):
    print(f"generate pair idx:{sub_sample_ids[0]}-idx{sub_sample_ids[1]}")
    if fill_strategy == 'pair_min':
        min_value = sub_sample.min()
        sub_sample = sub_sample.fillna(min_value)
        sub_sample.dropna(inplace=True, axis=1)
    else:
        raise "unknown fill strategy"
    return {
        "pair_feat": sub_sample.values,
        "pair_id": sub_sample_ids,
    }

def get_scale(_data, scale='minmax', **kwargs):
    if scale == 'no':
        pair_data = _data
    elif scale in ['minmax', 'log2_minmax']:
        scaler = MinMaxScaler()
        pair_data = scaler.fit_transform(_data)
    elif scale == 'std':
        scaler = StandardScaler()
        pair_data = scaler.fit_transform(_data)
    elif scale in ['qnorm', 'log2_qnorm']:
        try:
            import rpy2
            print(f"using rpy2 version:{rpy2.__version__}")
            if kwargs['r_home'] is not None:
                os.environ['R_HOME'] = kwargs['r_home']
            os.environ['R_USER'] = kwargs['r_user']
            print(kwargs['r_user'])
            from rpy2.robjects import r
        except Exception as e:
            print(e)
            print("unsupport this method without installing rpy2")
            print("please install rpy2 and check the R_USER and R_HOME path")
            exit(-2)
        ...
        r_data=limma.normalizeBetweenArrays(_data.T, method='quantile')
        try:
            pair_data =pandas2ri.ri2py(r_data).T
        except Exception as e:
            print("fucking rpy2 with 3.X version in Linux")
            pair_data=r_data.T
    else:
        raise 'unknown scale method'
    if "log2" in scale:
        pair_data = np.log2(pair_data)

    return pair_data

def get_distance(pair_data, dist_strategy='l2'):
    if dist_strategy == 'l2':
        d2 = pdist(pair_data, 'minkowski', p=2)[0]
    elif dist_strategy == 'l1':
        d2 = pdist(pair_data, 'minkowski', p=1)[0]
    elif dist_strategy == 'Mahalanobis':
        # if pair_data.shape[0] < pair_data.shape[1]:
        #     pca = PCA(n_components=pair_data.shape[0]-1)
        #     pair_data = pca.fit_transform(pair_data)
        d2 = pdist(pair_data.T, 'mahalanobis').mean()
    elif dist_strategy == 'Manhattan':
        d2 = pdist(pair_data, 'cityblock')[0]
    elif dist_strategy == 'Cosine':
        d2 = pdist(pair_data, 'cosine')[0]
    elif dist_strategy == 'Spearman':
        d2 = 1-scipy.stats.spearmanr(pair_data[0], pair_data[1])[0]
    else:
        raise 'unknown distance methods'
    return d2

def data_config(args):
    args.prepare_data_dir = os.path.join(args.logs_dir, f'step1_{args.joint_data}_scale')
    args.simulated_data_dir = os.path.join(args.logs_dir, f'step2_simulated_scale')

    if not PathManager.exists(args.prepare_data_dir):
        PathManager.mkdirs(args.prepare_data_dir)
        PathManager.mkdirs(args.simulated_data_dir)
        args.data_prepare = True
    # if args.runInferSingle:
    args.infer_log_dir = os.path.join(args.logs_dir, 'Inference', args.infer_datafile.split(".csv")[0])
